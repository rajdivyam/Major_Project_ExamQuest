...
const ValidEmail = require('../models/validEmail');
...
// Add allowed domain
authRoute.post('/add-domain', async (req, res) => {
    try {
        const { email } = req.body;
        ...
        const existing = await ValidEmail.findOne({ email: domain });
        if (existing) {
            return res.status(400).json({
                success: false,
        ...
        // Save domain into DB
        const newDomain = new ValidEmail({ email: domain });
...
authRoute.post('/verify-mail', async (req, res) => {
    try {
        const { email } = req.body;
        const domain = "@" + (email.split('@')[1] || '');
        const vaEmail = await ValidEmail.findOne({ email: domain });

        if (!vaEmail) {
            return res.json({
                success: false,
                message: 'Sorry, your email domain is not valid'
            });
        }
